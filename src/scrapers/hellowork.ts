        ...
        for (let pageNumber = 1; pageNumber <= 100; pageNumber += 1) {
            const entries = await page.locator(OFFER_SELECTOR).evaluateAll((links) =>
                links.map((link) => {
                    ...
                    const title =
                        card?.querySelector("h3 p")?.textContent?.replace(/\s+/g, " ").trim() ??
                    ...
                    return {
                        ...
                        contract:
                            visibleInfo.find((value) => /CDI|CDD|Intérim|Alternance|Freelance/i.test(value)) ?? null,
                        publishedAt:
