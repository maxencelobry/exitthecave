...
const SEARCH_URL = "https://candidat.francetravail.fr/offres/emploi/trappes/v236";
...
export class FranceTravailScrapper {
  ...
  private async applyLastDayFilter(page: Page): Promise<void> {
    ...
    const response = page.waitForResponse(
      (candidate) => candidate.url().includes("emploi.rechercheoffre.rechercheform"),
      { timeout: 15_000 },
    ).catch(() => undefined);
    ...
    this.log(`Résultat affiché : ${(await page.locator("h1").first().innerText().catch(() => "inconnu")).replace(/\s+/g, " ").trim()}`);
  }
  ...
  private async collectPages(page: Page, results: Map<string, FranceTravailResult>): Promise<void> {
    ...
    while (true) {
      ...
      const entries = await cards.evaluateAll((elements) => elements.map((element) => ({
        ...
        company: (() => {
          const subtext = element.querySelector(".subtext")?.textContent?.replace(/\s+/g, " ").trim() ?? "";
          const location = element.querySelector(".subtext span")?.textContent?.replace(/\s+/g, " ").trim() ?? "";
          return subtext.replace(location, "").replace(/\s*-\s*$/, "").trim();
        })(),
      ...
      await cards.nth(previousCount).waitFor({ state: "attached", timeout: 15_000 }).catch(() => undefined);
