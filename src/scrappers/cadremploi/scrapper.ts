...
export class CadremploiScrapper {
  async scrap(): Promise<CadremploiResult[]> {
    ...
    try {
      ...
      await page.evaluate(() => {
        const dateButton = [...document.querySelectorAll("button")].find((element) => element.textContent?.trim() === "Date");
  ...
  private async collectPages(page: Awaited<ReturnType<typeof connect>>["page"], results: Map<string, CadremploiResult>, expectedCount: number): Promise<void> {
    for (let pageNumber = 1; pageNumber <= 500; pageNumber += 1) {
      ...
      const hasNext = await page.$$eval("button", (buttons, nextPage) => buttons.some((button) => button.textContent?.trim() === String(nextPage)), pageNumber + 1);
  ...
  private log(message: string): void { console.log(`[Cadremploi] ${message}`); }
